...
impl PackageManagerBackend for NaptBackend {
    fn name(&self) -> &str {
        "NAPT"
    }

    fn is_available(&self) -> bool {
        has_binary("napt") && has_binary("dpkg")
    }
    ...
    async fn install(&self, package: &str) -> Result<(), String> {
        self.run_privileged("napt", &["install", package])
            .await
    }

    async fn remove(&self, package: &str) -> Result<(), String> {
        self.run_privileged("napt", &["remove", package])
            .await
    }

    async fn update(&self) -> Result<(), String> {
        self.run_privileged("napt", &["sync"]).await
    }
